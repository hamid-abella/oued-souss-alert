const StatsCard = ({ title, value, unit, icon: Icon, color, subtitle }) => (
  ...
  >
    <div style={{
      ...
      background: `${color || 'var(--color-primary)'}12`, pointerEvents: 'none',
    }} />
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <span style={{
        fontSize: '0.75rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)',
    ...
    <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
      <span style={{
        ...
        fontWeight: 800, color: color || 'var(--color-text)', lineHeight: 1,
      }}>{value ?? '—'}</span>
      {unit && <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>{unit}</span>}
    </div>
